...
export default function Network() {
  ...
  return (
    <div>
      <h2 className="text-xl sm:text-2xl font-semibold mb-6">Ağ Bilgileri</h2>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
          <div className="flex items-center gap-2 text-cyan-400 mb-2"><ArrowDown size={16} /> İndirme</div>
          <div className="text-2xl font-bold text-white">{fmt(stats?.speed_down_bps)}</div>
          <div className="text-xs text-gray-500 mt-1">Toplam: {fmtBytes(stats?.bytes_recv)}</div>
        </div>
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
          <div className="flex items-center gap-2 text-violet-400 mb-2"><ArrowUp size={16} /> Yükleme</div>
          <div className="text-2xl font-bold text-white">{fmt(stats?.speed_up_bps)}</div>
          <div className="text-xs text-gray-500 mt-1">Toplam: {fmtBytes(stats?.bytes_sent)}</div>
        </div>
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
          <div className="flex items-center gap-2 text-emerald-400 mb-2"><Activity size={16} /> Paketler</div>
          <div className="text-2xl font-bold text-white">{stats?.packets_recv || 0}</div>
          <div className="text-xs text-gray-500 mt-1">Gönderilen: {stats?.packets_sent || 0}</div>
        </div>
      </div>

      <div className="bg-gray-900 rounded-xl border border-gray-800 p-5 mb-6">
        <div className="text-sm font-medium text-gray-300 mb-3">Bant Genişliği (60sn)</div>
        <canvas ref={canvasRef} className="w-full h-20" />
        <div className="flex gap-4 mt-2 text-xs">
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-cyan-400" /> İndirme</span>
          <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-violet-400" /> Yükleme</span>
        </div>
      </div>

      <div className="bg-gray-900 rounded-xl border border-gray-800 p-5">
        <h3 className="font-medium mb-4 flex items-center gap-2"><Wifi size={16} /> Ağ Arayüzleri</h3>
