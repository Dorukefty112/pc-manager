...
import {
  ...
  Zap, CheckCircle, Info, TrendingUp, List
} from 'lucide-react'
...
export default function DepremUyari() {
  ...
  const [activeTab, setActiveTab] = useState('list') // map | list | stats (sadece mobilde geçerli)

  useEffect(() => {
    const handleResize = () => {
      const mobile = window.innerWidth < 768
      setIsMobile(mobile)
      if (!mobile) setActiveTab('list')
    }
  ...
  const renderStats = () => stats && (
    <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', flexDirection: isMobile ? 'column' : 'row' }}>
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: 12, width: '100%' }}>
        <StatCard
      ...
      {(kritikSayisi > 0 || tsunamiSayisi > 0) && (
  ...
  const renderList = () => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
      <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', marginBottom: 2 }}>
        {filtreli.length} deprem listeleniyor
          ...
          >
            ...
            <div
              ...
              onClick={() => {
                ...
                // Mobilde haritaya otomatik odaklansın diye eğer list sekmesindeysek haritaya uçabiliriz,
                // ya da direkt harita tabına geçirebiliriz:
                if (isMobile) {
                  setActiveTab('map')
                }
              }}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }} className="animate-fade-in">
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10, borderBottom: '1px solid var(--border)', paddingBottom: 12 }}>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <span style={{ fontSize: '0.62rem', color: 'var(--text-muted)' }}>Veri Kaynağı: Sismik Harita (Evrim Ağacı)</span>
          </div>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            ...
            <button onClick={fetchData} className="btn btn-secondary" style={{ padding: '6px 10px', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: 4 }}>
              <RefreshCw size={11} />
              <span style={{ fontSize: '0.65rem' }}>{countdown}s</span>
            </button>
            ...
            >
              {bildirim ? <Bell size={13} /> : <BellOff size={13} />}
            </button>
            ...
            >
              <Filter size={11} />
              {showFilters ? t('Kapat') : t('Filtrele')}
              {(minMag > 0 || selectedSources.length > 0 || onlyTsunami || viewFilter !== 'tum') && (
        ...
        {/* Görünüm Hızlı Filtresi (Sadece Tümü / İstanbul / Kritik) */}
        {!showFilters && (
          <div style={{ display: 'flex', gap: 6 }}>
            {['tum', 'istanbul', 'kritik'].map(f => {
              const labelMap = { tum: t('Tümü'), istanbul: t('İstanbul Çevresi'), kritik: t('Kritik Olanlar') }
              const isSel = viewFilter === f
              return (
                <button
                  key={f}
                  ...
                  style={{
                    ...
                    color: isSel ? 'var(--text)' : 'var(--text-secondary)',
        ...
        {/* Hata Mesajı */}
        {hata && (
          <div style={{ background: 'var(--red-glow)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 12, padding: '12px', color: 'var(--red)', fontSize: '0.8rem', display: 'flex', gap: 8 }}>
            <AlertTriangle size={14} style={{ flexShrink: 0 }} /> {hata}
            <button onClick={fetchData} style={{ marginLeft: 'auto', textDecoration: 'underline', background: 'none', border: 'none', color: 'inherit', fontSize: '0.78rem' }}>{t('Tekrar dene')}</button>
          </div>
        )}

        {/* Canlı Tab Seçici (Harita / Liste / İstatistik) */}
        <div style={{
          display: 'flex',
          ...
          {[
            { id: 'list', label: t('Liste'), icon: List },
            { id: 'map',  label: t('Harita'), icon: MapPin },
            { id: 'stats',label: t('İstatistik & Analiz'), icon: BarChart3 }
          ].map(tab => {
            const Icon = tab.icon
        ...
        {yukleniyor ? (
          ...
          <div className="tab-content" style={{ minHeight: 350 }}>
            {activeTab === 'map' && renderMap('calc(100vh - 195px)')}
