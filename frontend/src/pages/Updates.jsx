...
export default function Updates() {
  ...
  const upgrade = async () => {
    if (!confirm('Tüm güncellemeler yüklensin mi? Bu işlem biraz sürebilir.')) return
    try {
      const res = await api('/api/updates/upgrade', { method: 'POST' })
      setMessage(res.message || 'Güncelleme başlatıldı')
    } catch (e) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl sm:text-2xl font-semibold">Güncellemeler</h2>
        ...
        <div className="bg-gray-900 rounded-xl border border-gray-800 p-5 mb-4">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <span className="text-lg font-medium">{data.count} güncelleme mevcut</span>
            </div>
            {data.count > 0 && (
              <button onClick={upgrade} className="flex items-center gap-2 px-4 py-2 bg-cyan-700 rounded-lg hover:bg-cyan-600 text-sm font-medium">
                <Download size={14} /> Tümünü Güncelle
              </button>
            )}
          </div>
          {data.error && (
